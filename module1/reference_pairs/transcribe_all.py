...
def main() -> None:
    mc = MooreClient()
    wav_files = sorted(p for p in WAV_DIR.glob("*.wav") if not p.name.startswith("clip"))
    ...
    for wav in wav_files:
        ...
        while start < duration:
            ...
            try:
                ...
                print(f"  [{start:.0f}s+] {text[:80]}...")
                pieces.append(text)
            except Exception as exc:
                print(f"  [{start:.0f}s+] ERREUR: {exc}")
                pieces.append(f"[ERREUR à {start:.0f}s: {exc}]")
            start += CHUNK_S
